 ...
 */
const getTextBlocks = async (s3Bucket, s3Name) => {
  const params = {
    Document: {
      S3Object: {
        Bucket: s3Bucket,
...
const getRawParagraphs = (blocks) => { // TODO: RAYADA, COMPROBAR CON OTROS DOCUMENTOS
  ...
  for (let i = 1; i < lines.length; i++) {
    const currentTopDifference = lines[i].Geometry.BoundingBox.Top - lastLineTop
 ...
 * @returns {Promise<Array<String>>} Array of keywords extracted from the text
 */
